...
void DXResourceManager::CreateMaterial()
{
	...
	/// Ending Scene
	LoadResource<Material>(L"ClearTX", m_basePath + L"Texture/ending/ClearSceen.png", m_device);
	LoadResource<Material>(L"GameOverTX", m_basePath + L"Texture/ending/GameOverSceen.png", m_device);
...
void DXResourceManager::CreateGeoMetry()
{
	...
	CreateUIRect(100, 100, L"HideInterBT_Buffer");
}
...
#pragma region UI
	...
	// ClearUI
	{
		auto model = LoadResource<Model>(L"ClearUI");

		auto mesh = LoadResource<Mesh>(L"CanvasBuffer");
		model->m_meshes.push_back(mesh);

		auto textureBG = LoadResource<Material>(L"ClearTX");
	...
	// GameOverUI
	{
		auto model = LoadResource<Model>(L"GameOverUI");

		auto mesh = LoadResource<Mesh>(L"CanvasBuffer");
		model->m_meshes.push_back(mesh);

		auto textureBG = LoadResource<Material>(L"GameOverTX");
...
#pragma region Character
	...
	// [Character] 상인
	{
		...
		modelLoader->LoadAnimation(L"MerchantShouting", "Character/merchant_shouting.fbx", AnimationState::Loop);
		auto vertexShader = LoadResource<VertexShader>(L"DefferedSkinnedVS");
		model->m_vertexShader = vertexShader;
		model->m_pixelShader = defferedDefaultPS;
		model->m_vertexCbufferTypes.push_back(VertexCBufferType::Transform);
