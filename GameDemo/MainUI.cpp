...
void MainUI::Update(double dt)
{
	...
	{
		// 훔치기 영역
		if (260.f <= mousePos.x && mousePos.x <= 610.f
			&& 610.f <= mousePos.y && mousePos.y <= 676.f)
		{
		...
		// 만든이 영역
		if (260.f <= mousePos.x && mousePos.x <= 550.f
			&& 750.f <= mousePos.y && mousePos.y <= 826.f)
		{
			SoundManager::GetInstance()->PlaySFX("Click.mp3");
			m_isOnCredit = true;
		}
		...
		// 안훔치기 영역
		if (260.f <= mousePos.x && mousePos.x <= 550.f
			&& 890.f <= mousePos.y && mousePos.y <= 965.f)
		{
			SoundManager::GetInstance()->PlaySFX("Click.mp3");
			m_isOnExit = true;
		}
